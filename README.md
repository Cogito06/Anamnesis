...
```bash
python ~/.claude/skills/cairn/scripts/graph.py cairn
```

生成 `cairn/graph.html`——单文件、零外部依赖、力导向布局，用浏览器直接打开。

图能看出索引看不出的四件事：**哪些簇是孤岛**、**哪些簇之间只有一根线连着**
（那根线通常就是理解刚打通的地方，画成虚线标为「桥」）、**哪些片段谁也不认识**
（孤儿会被推到边缘）、**哪些片段你已经知道该写但还没写**。

最后一条借自 Obsidian 的 unresolved 节点：指向不存在片段的 `[[链接]]` 不丢掉，
画成空心虚线圈。死链在 cairn 里不是错误，是一个**意图**——写下
`[[还没写的片段]]` 说明你已经知道那块该有东西。它和孤儿正好成对：
孤儿有内容没连接，幽灵有连接没内容。

有未解决问题的节点带圆环和计数，点节点看钩子、遗留问题、材料和链接。
CSS 变量沿用 Obsidian 的命名（`--graph-node` / `--graph-line` / `--graph-text` /
`--graph-node-unresolved`），调过 Obsidian 主题的人拿到文件就知道改哪里。

配色只用三个经过 CVD 全配对验证的色相——节点图里任意两簇都可能相邻，
适用「全配对」而非「相邻配对」标准，实测第四个色相（黄↔橙正常视觉 ΔE 13.7）会挂。
超出三个的簇归中性灰，身份靠节点标签和簇名标签承载，颜色从不单独表意。
