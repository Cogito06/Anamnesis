...
def main():
    ...
    for f in files:
        ...
        rows.append((sort_key, f"- [{slug}] {'/'.join(concepts[:MAX_CONCEPTS])} "
    ...
    body_lines = [r[1] for r in rows]
    (root / "INDEX.md").write_text(
        "\n".join([MARKER, "# Cairn Index", ""] + body_lines) + "\n", encoding="utf-8")

    n_links = sum(len(v) for v in out_links.values())
    total_bytes = sum(len(l.encode()) + 1 for l in body_lines)
    avg = total_bytes // max(len(body_lines), 1)
    print(f"索引已重建：{len(rows)} 条片段，{total_open} 个未解决问题，{n_links} 条片段间链接")
    print(f"  索引正文 {total_bytes} 字节，平均每行 {avg} 字节")
    for w in warn:
        print(f"  警告 {w}")
    if orphans:
        print(f"  孤儿片段（没有任何进出链接，可能还没接进你的理解网络）：{', '.join(orphans)}")
    if avg * len(rows) > 12000:
