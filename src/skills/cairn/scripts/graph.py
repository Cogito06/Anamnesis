...
def collect(root):
    ...
    # 先收集死链。死链在 cairn 里不是错误，是一个**意图**——写下
    # [[还没写的片段]] 说明已经知道那块该有东西。丢掉它，图就只画了
    # 「已经想通的」，画不出「知道自己缺什么」。所以画成幽灵节点。
    # 它和孤儿正好成对：孤儿是有内容没连接，幽灵是有连接没内容。
    raw = {}
    for slug in nodes:
        raw[slug] = sorted(set(LINK_RE.findall(
            (frag_dir / (slug + ".md")).read_text(encoding="utf-8"))))

    ghost_from = {}
    for slug, targets in raw.items():
        for t in targets:
            if t not in slugs and t != slug:
                ghost_from.setdefault(t, []).append(nodes[slug]["cluster"])
    for g, cls in ghost_from.items():
        # 跟着引用它的片段所在的簇，这样它在空间上就落在「本该在的位置」
        nodes[g] = {"id": g, "cluster": max(sorted(set(cls)), key=cls.count),
                    "hook": "", "concepts": [], "open": [], "date": "",
                    "bytes": 0, "refs": [], "ghost": True,
                    "from": sorted(set(s for s, t in
                                       ((s, t) for s in raw for t in raw[s]) if t == g))}
    for n in nodes.values():
        n.setdefault("ghost", False)
        ...
        for t in targets:
            ...
            if key in seen:
                ...
                seen[key] = {"s": key[0], "t": key[1], "mutual": False,
                             "ghost": nodes[key[0]]["ghost"] or nodes[key[1]]["ghost"],
...
def main():
    ...
    bridges = sum(1 for e in edges if e["bridge"] and not e["ghost"])
    real = [n for n in nodes if not n["ghost"]]
    ghosts = [n["id"] for n in nodes if n["ghost"]]
    orphans = [n["id"] for n in real if n["deg"] == 0]
    print(f"已生成 {out}")
    print(f"  {len(real)} 个片段，{len(edges)} 条边（{bridges} 条跨簇的桥），{len(clusters)} 个簇")
    if ghosts:
        print(f"  幽灵节点（被链接但还不存在的片段，图上画成空心）：{', '.join(ghosts)}")
    if len(clusters) > 4:
