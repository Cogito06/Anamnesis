...
MAX_BYTES=32768   # 约 8k token；超过说明该分片了
...
n=$(grep -c '^- \[' "$idx" 2>/dev/null | tr -d ' ')

echo "## Anamnesis —— 已归档的认知片段（$CAIRN_DIR/INDEX.md，共 ${n:-0} 条）"
echo
size=$(wc -c < "$idx" | tr -d ' ')
if [ "${size:-0}" -gt "$MAX_BYTES" ]; then
  head -c "$MAX_BYTES" "$idx"
  printf '\n\n（索引超过 %s 字节已截断——该按 course 分片了，见 ARCHITECTURE.md 第 3 节）\n' "$MAX_BYTES"
else
  cat "$idx"
fi
...
echo
echo "讨论到索引里已有的主题时，先读 $CAIRN_DIR/fragments/ 下对应片段的全文再往下推，不要重新推导用户已经想通的部分。索引里那句钩子是判断该不该展开的依据，不是片段的全部内容。"
exit 0
